...
Entretanto, na música \textit{Message In A Bottle}, o mesmo não ocorre. Sempre que o músico remoto tocar uma transição, haverá um erro na previsão, pois nenhum estado aponta para transições. Porém, vale mencionar que esse erro será corrigido assim que possível - precisamente, a correção tem a duração de meio compasso, como determinado como \textit{offset} para cada janela. É possível fazer uma comparação desse estado à sensação de "teletransporte", mencionado na \secref{sec:client_side_prediction}, na implementação original de \textit{client-side prediction} para jogos.
